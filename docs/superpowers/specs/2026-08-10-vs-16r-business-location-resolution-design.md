# VS-16R Business Location Resolution — Design Specification

## Problem

Live Expo Go validation showed that URL-first discovery can return provider-generic metadata (`Bolt Food`) instead of the venue, and then forces the owner to type country, timezone and currency manually. This creates two trust failures: wrong Business identity and non-canonical location metadata.
