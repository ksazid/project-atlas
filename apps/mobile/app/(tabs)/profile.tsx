...
const empty: BusinessProfile = { description: '', address: '', website: '', socialChannels: '', businessChannels: '', hours: '', language: 'English', source: 'owner', ownerConfirmed: true };
...
export default function ProfileScreen() {
  ...
  useEffect(() => { void (async () => { try { const s = await loadSession(); if (s?.businessId) setForm(await getProfile(s.accessToken, s.businessId)); } catch { setMessage('Profile is unavailable. You can retry later.'); } finally { setBusy(false); } })(); }, []);
